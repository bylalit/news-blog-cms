...
const dashboard = async (req, res, next) => { 
    try {
        ...
        const categorieCount = await categoryModel.countDocuments();
        const userCount = await userModel.countDocuments();

        res.render('admin/dashboard', { role: req.role, fullname: req.fullname, articleCount, categorieCount, userCount });
    } catch (error) {
        next(error);
    }
};
...
const saveSettings = async (req, res, next) => { 
    ...
    try {
        ...
        if (setting) {
            ...
            await settingModel.create({ website_title, footer_description, website_logo });
        }
    } catch (error) {
        next(error);
    }
